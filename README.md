...
Safe defaults keep `TELEGRAM_WOLF_BRIEFING_ENABLED=false` and `TELEGRAM_WOLF_BRIEFING_PUBLIC_ENABLED=false`. When disabled, the admin command is audited but not sent. Public briefing delivery is not enabled in this phase.
...
- The Phase 46B Telegram Wolf Briefing layer is manual admin reporting only. It formats existing scan/watch/lifecycle state; it does not create signals, convert rejected or near-miss rows into alerts, alter scanner gates, place orders, call private exchange APIs, expose secrets, invent market data, withdraw funds, or transfer funds.
