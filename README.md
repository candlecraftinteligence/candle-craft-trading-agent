...
## Phase 44E - Replay Dataset Coverage Report

Phase 44E adds deterministic, read-only coverage reporting over the Phase 44C replay dataset rows and Phase 44D-style quality concepts. It shows how well the dataset covers sources, artifact types, row types, symbols, statuses, lifecycle buckets, setup research buckets, strategy modes, directions, grades, rejection gates, outcome statuses, replay readiness, trade idea/alert/journal presence, result coverage, missing-data coverage, and unverified-data coverage.

This is audit/research-only. Coverage is data and research coverage, not profitability, not setup confidence, and not a signal-quality score. Rejected and no-setup rows are valid research rows because they explain what the scanner rejected and which setup gates need better coverage.

Run coverage reporting with default local artifacts:

```powershell
.\.venv\Scripts\python.exe scripts\analyze_replay_dataset_coverage.py
```

JSON mode:

```powershell
.\.venv\Scripts\python.exe scripts\analyze_replay_dataset_coverage.py --json
```

Strict mode:

```powershell
.\.venv\Scripts\python.exe scripts\analyze_replay_dataset_coverage.py --strict
```

Analyze an exported JSONL replay dataset:

```powershell
.\.venv\Scripts\python.exe scripts\analyze_replay_dataset_coverage.py --input replay_exports\latest_replay_dataset.jsonl --input-format jsonl --json
```
...
## Phase 44G - Replay Research Report Pack

Phase 44G adds a deterministic, read-only report pack over the Phase 44A-F audit/export/quality/coverage/failure taxonomy stack. It produces one concise markdown research report and one JSON-serializable summary/report object from the default local replay artifacts or explicit input artifacts.

This is audit/research-only. It does not execute replay, create signals, place trades, call exchanges, send Telegram messages, mutate artifacts, update scanner results, change lifecycle states, write performance memory, alter database records, or change setup gates. Missing values remain `N/A`, unreliable values remain `Unverified`, and market data, outcomes, profitability, and hidden failure reasons are never invented.

Run the default markdown report:

```powershell
.\.venv\Scripts\python.exe scripts\generate_replay_research_report.py
```

JSON mode:

```powershell
.\.venv\Scripts\python.exe scripts\generate_replay_research_report.py --json
```

Write a local markdown report:

```powershell
.\.venv\Scripts\python.exe scripts\generate_replay_research_report.py --markdown --output replay_reports\latest_replay_research_report.md
```

Write a local JSON report:

```powershell
.\.venv\Scripts\python.exe scripts\generate_replay_research_report.py --json --output replay_reports\latest_replay_research_report.json
```

Generated `replay_reports/*.md` and `replay_reports/*.json` files are local research artifacts and should not be committed.

Research priorities in the report are review priorities only. They are not trading signals, not setup approvals, not profitability claims, and not recommendations to loosen scanner, strategy, pullback, OB/FVG, fib, RR, Trust Meter, risk, scoring, lifecycle, portfolio, regime, or alert gates.

## Phase 45A - Historical Replay Validation Scaffolding

Phase 45A adds deterministic, read-only scaffolding for future historical replay validation. It consumes Phase 44C replay dataset rows from local scanner, watch-state, lifecycle, and performance-memory artifacts, then produces validation candidates, a replay timeline, readiness diagnostics, and a JSON-serializable validation plan.

This is scaffold/plan-only. It does not run historical PnL, execute replay outcome scoring, create signals, place trades, call exchanges, send Telegram messages, mutate artifacts, alter lifecycle state, update performance memory, change scanner behavior, or weaken setup gates. It does not calculate win rate, edge, profitability, or expectancy; missing values remain `N/A`, and market data or outcomes are never invented.

Run the default validation plan summary:

```powershell
.\.venv\Scripts\python.exe scripts\build_replay_validation_plan.py
```

JSON mode:

```powershell
.\.venv\Scripts\python.exe scripts\build_replay_validation_plan.py --json
```

Write a local JSON validation plan:

```powershell
.\.venv\Scripts\python.exe scripts\build_replay_validation_plan.py --json --output replay_validation\latest_validation_plan.json
```

Generated `replay_validation/*.json` files are local validation planning artifacts and should not be committed.

Rejected and no-setup rows are preserved as negative examples for research. They remain candidates so later phases can validate historical outcomes without biasing the dataset toward only alerts or trade ideas.

## Phase 45B - Historical Replay Event Sequence Validator

Phase 45B adds a deterministic, read-only event sequence validator for Phase 45A replay validation plans. It groups replay timeline events by stable setup, trade idea, alert, scan/run, candidate, or symbol-row identity, then checks event ordering, duplicate event identities, missing timestamps, missing symbols, unknown statuses, suspicious lifecycle transitions, and replay-readiness blockers.

This is audit/validation-only. It validates timeline and sequence consistency only. It does not run historical PnL, calculate win rate, claim expectancy, calculate edge, create signals, place trades, call exchanges, send Telegram messages, mutate artifacts, alter lifecycle state, update performance memory, change scanner behavior, or weaken setup gates. Missing values remain `N/A`, unreliable data remains `Unverified`, and hidden prices, candles, outcomes, or lifecycle transitions are never inferred.

Run the default event sequence validation summary:

```powershell
.\.venv\Scripts\python.exe scripts\validate_replay_event_sequences.py
```

JSON mode:

```powershell
.\.venv\Scripts\python.exe scripts\validate_replay_event_sequences.py --json
```

Write a local JSON event sequence validation result:

```powershell
.\.venv\Scripts\python.exe scripts\validate_replay_event_sequences.py --json --output replay_validation\latest_event_sequence_validation.json
```

Generated `replay_validation/*.json` files are local generated validation artifacts and should not be committed.

Rejected and no-setup rows are preserved as negative examples for research. They are valid audit evidence, not system failures merely because no trade happened.

## Phase 45C - Replay Outcome Field Readiness Contract

Phase 45C adds a deterministic, read-only outcome field readiness contract for replay artifacts. It validates the minimum identity, trade setup, terminal outcome, and negative-example fields needed before Candle Craft can make future historical replay outcome claims.

This is contract/readiness-only. It defines required fields and reports missing data as warnings/readiness blockers; it does not calculate PnL, win rate, expectancy, edge, or profitability. It does not create signals, place trades, call exchanges, send Telegram messages, mutate artifacts, change lifecycle state, or alter scanner, strategy, scoring, risk, portfolio, regime, alert, setup, or validation gates. Missing values remain `N/A`, unreliable values remain `Unverified`, and hidden prices, candles, outcomes, or market data are never inferred.

Run the default outcome readiness summary:

```powershell
.\.venv\Scripts\python.exe scripts\audit_replay_outcome_readiness.py
```

Print the readiness result as JSON:

```powershell
.\.venv\Scripts\python.exe scripts\audit_replay_outcome_readiness.py --json
```

Write a local JSON outcome readiness result:

```powershell
.\.venv\Scripts\python.exe scripts\audit_replay_outcome_readiness.py --json --output replay_validation\latest_outcome_readiness.json
```
...
## Phase 46A Telegram Admin Draft Router

Phase 46A adds an admin-only Telegram reporting layer after scan completion. It builds a compact admin scan report from the saved scan/manifest context, persists local draft records under `scan_runs/admin_drafts/`, and optionally sends one report to `TELEGRAM_ADMIN_CHAT_ID` only when `TELEGRAM_ADMIN_ENABLED=true`, `TELEGRAM_DRY_RUN=false`, and admin credentials are present.

Safe defaults keep `TELEGRAM_ADMIN_ENABLED=false` and `TELEGRAM_DRY_RUN=true`. Missing Telegram credentials do not fail scans; they produce a warning and local draft artifacts. `TELEGRAM_PUBLIC_CHANNEL_ID` and `TELEGRAM_VIP_CHANNEL_ID` are configuration placeholders only in this phase and are not used for sending.

This is admin review and draft persistence only. It does not auto-post public or VIP alerts, implement payments, create a Telegram Mini App, place orders, use exchange execution, weaken setup gates, convert near misses into valid setups, withdraw funds, transfer funds, or invent market data.

## Phase 46B Telegram Wolf Briefing

Phase 46B adds an admin-only `🐺 Wolf Briefing` Telegram command/menu action. It formats the latest persisted scan/watch/lifecycle state into a concise market-state briefing with `Market Mood`, signal quality, best action, active signal count, watchlist count, near-miss count, rejected setup count, and a short focus list from existing active/watchlist/near-miss rows only.

Safe defaults keep `TELEGRAM_WOLF_BRIEFING_ENABLED=false` and `TELEGRAM_WOLF_BRIEFING_PUBLIC_ENABLED=false`. When disabled, the admin command is audited but not sent. Public user access is disabled by default.

This is reporting and formatting only. It does not run a scanner, create signals, promote rejected or near-miss setups, weaken setup gates, place orders, call exchange execution APIs, withdraw funds, transfer funds, expose secrets, or invent market data. Missing values render as `N/A`, unreliable values render as `Unverified`, and the public-facing copy says `Market Mood` instead of `Regime`.

Scheduled morning/evening Wolf Briefings are a future step. This phase does not add a long-running scheduler loop.

## Phase 46C Admin-Published Wolf Briefing

Phase 46C adds a private admin preview-and-publish flow for Wolf Briefing. Admin `/wolf` and the `🐺 Wolf Briefing` admin button render a private `🐺🟠 WOLF BRIEFING PREVIEW` with inline controls to send, refresh, or cancel. The preview is never sent to groups, channels, or public users.

Manual channel publishing is gated separately by `TELEGRAM_WOLF_BRIEFING_CHANNEL_PUBLISH_ENABLED=false` and `TELEGRAM_WOLF_BRIEFING_CHANNEL_ID=`. When the explicit Wolf channel ID is empty, the publish action may fall back to `TELEGRAM_PUBLIC_CHANNEL_ID`; it never falls back to the admin chat. The publish action sends only one clean text briefing to the configured public/signal channel, with no inline buttons, persistent keyboard, admin menu, public bot menu, or preview/admin diagnostics.

`TELEGRAM_WOLF_BRIEFING_PUBLIC_ENABLED` is not used for channel publishing. That flag only controls whether public users may request a private, public-safe Wolf Briefing from the bot if enabled in the future. Public users cannot publish, and group/supergroup/channel command updates are ignored by the command listener.
...
- `--universe binance_usdt_perp_top_market_cap` uses CoinPaprika's no-key `/v1/tickers` feed, which the provider documents as active cryptocurrencies with explicit global `rank` and `quotes.USD.market_cap` fields. CCI first retains only valid absolute ranks `1..N`, rejects ambiguous duplicate provider tickers, and only then intersects that set with Binance `/fapi/v1/exchangeInfo` contracts that are `TRADING`, `PERPETUAL`, USDT-quoted, and `underlyingType=COIN`. Missing Binance matches make the final universe smaller; ranks above N never backfill it.
- In market-cap mode, `--universe-size N` is an absolute global cryptocurrency-rank boundary. In the existing volume/tradability modes it remains the requested post-sort symbol count.
- `--max-symbols N` is a final cap, never a population target. Lifecycle state, continued watches, saved-run restoration, adaptive priority, symbol health, and cooldown metadata may reorder or reduce strict market-cap membership but cannot add symbols outside it; historical lifecycle rows remain stored.
- `--min-quote-volume N` is optional and defaults to `0`; in market-cap mode it can remove a Binance match but never replace it with rank `>N`.
- Market-cap resolution has no fallback/cache universe. Provider, exchange-contract, empty-payload, or malformed-ranking failures stop resolution rather than scanning an unrelated Binance universe.
- Scanner headers and saved JSON include the selected universe mode, source, requested size/cap, resolved symbols, rank-boundary/provider counts, Binance matches, missing/invalid ranks, rank-`>N` exclusions, ambiguity counts, cache status, and top resolved symbols where available.
...
- The Phase 44G replay research report pack is read-only audit/research only. It consolidates explicit replay audit/export/quality/coverage/failure metrics into markdown or JSON reports only; it does not execute replay, create signals, place trades, call exchanges, send Telegram messages, mutate artifacts, alter gates, infer hidden reasons, calculate profitability, invent market data, withdraw funds, or transfer funds.
...
- The Phase 46B/46C Telegram Wolf Briefing layer is manual admin reporting and admin-approved channel publishing only. It formats existing scan/watch/lifecycle state; it does not create signals, convert rejected or near-miss rows into alerts, alter scanner gates, place orders, call private exchange APIs, expose secrets, invent market data, withdraw funds, or transfer funds.
